...
App.prototype.setRouteConfigurator = function(routeConfigurator) {
	...
	routes.forEach(function(route) {
		...
		if (!actionClass) {
			...
			if (alias) {
				self.serveStatic(route.getPath(), path.join(process.cwd(), 'dist', alias));
			}
		...
		var Class = self.classLoader.loadClass(actionClass);
		...
		action.setTemplateEngine(self.getTemplateEngine());

		if (route.getHttpMethod()) {
			var verb = route.getHttpMethod().toLowerCase();
			self.engine[verb](route.getPath(), middleware.bind(action));
		}
		else {
			var macroManager = new madvoc.RouteMacroManager(route.getPath());

			if (route.getPath() === '/') {
				self.engine.get(route.getPath(), middleware.bind(action));
			}
			else {
				self.engine.use(
					macroManager.replaceMacros(':$1($2)', ':$1'),
					function(req, res, next) {
						if (macroManager.match(req.originalUrl)) {
							middleware.apply(action, arguments);
						}
						else {
							next('route');
						}
